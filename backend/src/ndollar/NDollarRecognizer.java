...
/**
 * This class is the center of the dollarN recognition. Indeed, this is where a gesture is recognized, gestures are loaded and saved
 * @author robin
 *
 */
...
public class NDollarRecognizer {
	...
	private Hashtable<String, Multistroke> _gestures; //Hashtable where gestures in memory are saved under the Multistroke format
	...
	//This is the method called to recognize a set of points
	public NBestList Recognize(Vector<PointR> points, int numStrokes)
	{
			...
			if (!NDollarParameters.getInstance().MatchOnlyIfSameNumberOfStrokes
				...
				for (Gesture p : ms.Gestures) {
					...
					if (!NDollarParameters.getInstance().DoStartAngleComparison
						...
						//Automatically use the GoldenSectionSearch. If it doesn't work, use the OptimalCosineDistance in the NDollarRecognizerAncien class
	...
	//Gestures in memory are loaded here
